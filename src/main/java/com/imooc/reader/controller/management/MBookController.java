...
public class MBookController {
    ...
    public Map list(Integer page,Integer limit){
        ...
        IPage<Book> pageObjects = bookService.paging(null,null,page,limit);
        Map result = new HashMap();
        result.put("code","0");
        result.put("msg","success");
        //当前页面数据
        result.put("data", pageObjects.getRecords());
        //未分页时记录总数
        result.put("count",pageObjects.getTotal());
        return result;
    }
    @GetMapping("/id/{id}")
    @ResponseBody
    public Map selectById(@PathVariable("id") Long booId){
        Book book = bookService.selectById(booId);
        Map result = new HashMap();
        result.put("code","0");
        result.put("msg","success");
